...
const linkers = ["ccache", "ilink", "link", "ld", "ccld", "gcc", "clang\\+\\+", "clang", "cc", "g\\+\\+", "c\\+\\+"];
...
function parseMultipleSwitchFromToolArguments(args, sw, removeSurroundingQuotes = true) {
    ...
    function anythingBetweenQuotes(fullyQuoted) {
        ...
        let str = anythingBetweenQuotesBasicPattern("`") + '|' + anythingBetweenQuotesBasicPattern('"') + (fullyQuoted ? "" : '|' + anythingBetweenQuotesBasicPattern("'"));
        return str;
    }
            ...
            while (index < numberOfLines && chunkIndex <= chunkSize) {
                ...
                if (!line.includes("$")) {
                    ...
                    if (targetBinaryTool) {
                        ...
                        let splitArgs = targetBinaryTool.arguments ? yield parseAnySwitchFromToolArguments(targetBinaryTool.arguments, []) : [];
